...
var debug = require('debug-logger')('superlogin:routes');
...
module.exports = function(config, router, passport, user) {
      ...
      user.create(req.body, req)
        .then(function (newUser) {
          if(config.getItem('security.loginOnRegistration')) {
            return user.createSession(newUser._id, 'local', req)
              .then(function (mySession) {
                res.status(200).json(mySession);
              }, function (err) {
                return next(err);
      ...
      user.resetPassword(req.body, req)
        .then(function (currentUser) {
          if(config.getItem('security.loginOnPasswordReset')) {
            return user.createSession(currentUser._id, 'local', req)
              .then(function (mySession) {
                res.status(200).json(mySession);
              }, function (err) {
                return next(err);
  ...
  router.post('/change-email',
    ...
    function (req, res, next) {
      ...
      user.changeEmail(req.user._id, req.body, req)
        .then(function () {
  ...
  router.post('/update/:fieldId',
    passport.authenticate('bearer', {session: false}),
    function (req, res, next) {
      var fieldId = req.params.fieldId;
      user.updateCustomField(req.user._id, fieldId, req.body)
        .then(function (){
          res.status(200).json({ok: true, success: fieldId+' changed'});
        }, function (err) {
          next(err);
        });
    });  
