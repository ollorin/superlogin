...
var debug = require('debug-logger')('superlogin:dbauth');
...
module.exports = function (config, userDB, couchAuthDB) {
  ...
  function storeKey(username, key, password, expires, refreshed, roles) {
        ...
        if (designDocs && designDocs instanceof Array) {
          designDocs.forEach(function(ddName) {
            ...
            if(dDoc) {
              ...
              debug.warn('Failed to locate design doc: ' + ddName);
            }
          });
        }
  ...
  this.getDesignDoc = function(docName) {
    ...
    try {
      ...
      debug.warn('Design doc: ' + designDocDir + '/' + docName + ' not found.');
      designDoc = null;
    }
    return designDoc;
  };
