...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.updateCustomField = function(user_id, field_id, value){
    var customFields = config.getItem('userModel.whitelist');
    if(customFields && (customFields.indexOf(field_id) != -1)){
      return userDB.get(user_id)
        .then(function(userDoc){
          userDoc[field_id] = value;
          return userDoc;
        })
    ...
    return BPromise.reject({
      error: 'Field operation not permitted',
      status: 400
    });
  };
  ...
  this.changeEmail = function(user_id, form, req) {
    if(!form.currentPassword){
      return BPromise.reject({error: 'Email change failed', message: 'You must supply your current password in order to change email.', status: 400});
    }
    req = req || {};
    if(!req.user) {
    ...
    var user;
    return self.validateEmail(form.newEmail)
      .then(function(err) {
        if(err) {
      ...
      function(err) {
        return BPromise.reject(err || {error: 'Email change failed', message: 'The current password you supplied is incorrect.', status: 400});
      });
};
